...
retry_times 5 30 "kubectl apply -f e2e-tests/data/cr.yaml"

retry_times 30 3 "kubectl get deploy/${CLUSTER_ID}"

kubectl wait --for=condition=Available --timeout=${TIMEOUT}s deploy/${CLUSTER_ID} || cleanup_and_exit 1
...
kubectl wait --for=condition=Ready --timeout=${TIMEOUT}s pod/$jm_pod_name || cleanup_and_exit 1

wait_for_logs $jm_pod_name "Rest endpoint listening at" ${TIMEOUT}
...
kubectl exec $jm_pod_name -- /bin/sh -c "kill 1" || cleanup_and_exit 1

# Check the new JobManager recovering from latest successful checkpoint
wait_for_logs $jm_pod_name "Restoring job $job_id from Checkpoint" ${TIMEOUT}
